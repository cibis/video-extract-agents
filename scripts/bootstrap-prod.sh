#!/usr/bin/env bash
# scripts/bootstrap-prod.sh  (gitignored — never commit)
#
# One-time provisioning of the video-extract-prod Azure environment.
...
# prod requires image_tag explicitly (no default in variables.tf)
export TF_VAR_image_tag="latest"   # placeholder images use latest; CI will update to real SHA after first deploy

# ─── Run ──────────────────────────────────────────────────────────────────────

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TF_DIR="$SCRIPT_DIR/../infrastructure/terraform/envs/prod"

# ─── Logging setup ────────────────────────────────────────────────────────────
LOG_DIR="$SCRIPT_DIR/../gitlab-logs/terraform"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/bootstrap-prod-$(date +%Y%m%d_%H%M%S).log"
...
ACR_NAME=$(az acr list --resource-group video-extract-prod --query "[0].name" --output tsv)
ACR_LOGIN_SERVER=$(az acr show --name "$ACR_NAME" --resource-group video-extract-prod --query loginServer --output tsv)
ACR_PASSWORD=$(az acr credential show --name "$ACR_NAME" --resource-group video-extract-prod --query "passwords[0].value" --output tsv)
ACR_USERNAME=$(az acr credential show --name "$ACR_NAME" --resource-group video-extract-prod --query username --output tsv)
