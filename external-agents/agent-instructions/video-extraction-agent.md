...
Call `ingest_video` first to upload the video and create the keyframe index.

**LibreChat** — LibreChat embeds the file URL in the message context. Use it directly:

```json
{
  "source_url": "<URL from LibreChat file attachment>",
  "job_id": "<job_id>",
  "session_id": "<session_id>"
}
```

**Claude Desktop** — Claude Desktop provides a local file path. Use the `azure-storage` MCP server to upload the video to Azurite first, then call `ingest_video`:

1. Upload: `azure_storage.upload_blob(container="videos", blob_name="external-uploads/<filename>", file_path="<local path>")`
2. The upload returns `http://localhost:10000/devstoreaccount1/videos/external-uploads/<filename>`
3. Call: `ingest_video(source_url="http://azurite:10000/devstoreaccount1/videos/external-uploads/<filename>", job_id=..., session_id=...)`
   - Note: replace `localhost:10000` with `azurite:10000` — `ingest_video` handles the remap automatically, but be explicit.
...
1. `ingest_video` (Phase 0)
2. `extract_frames` → `frames_asset`
3. `detect_motion_sports(frames_asset)` → `detection_result_asset`
4. `query_asset(detection_result_asset, "$.segments[*]")` → read segments
5. `write_segments_asset(segments)` → `segments_asset`
6. `extract_clips_bulk(segments_asset, video_url)` → `clip_list_asset`
7. `merge_clips(clip_list_asset)` → `output_url`
8. Report `output_url` to user
