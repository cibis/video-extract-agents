...
description: Use this skill whenever the user wants to extract video clips, create highlight reels, or compile specific moments from a video file. This includes sports highlights, jump detection, motion detection, object detection, speech/audio transcription, scene analysis, and any natural language prompt describing what to find or extract from a video. If the user attaches or mentions a video file, or asks to process video content through an extraction pipeline, use this skill. Also use it when the user asks to detect motion, find objects, transcribe audio, or merge clips — even if they don't say "highlight reel" explicitly.
---
...
Returns `upload_url`. Present it to the user as a clickable link that opens in a new browser tab:

> Please open this link to upload your video file: **[Upload Video]** — paste the `upload_url` from the tool result into your browser (opens in a new tab).
>
> Click the link, upload your file(s) in the browser, then come back and let me know when done.
...
11. Report `output_url` to user. If there are any occurrences of `azurite:10000` in the `output_url` the replace them with `localhost:10000` before reporting.
