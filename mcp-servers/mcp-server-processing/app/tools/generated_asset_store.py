...
async def append_to_clip_list(
    session_id: str | None,
    job_id: str,
    clip_url: str,
) -> str:
    """Append a clip URL to the running clip_list.json blob. Returns the blob URL."""
    blob_path = _blob_path(session_id, job_id, "clips", "clip_list.json")
    async with BlobServiceClient.from_connection_string(
        settings.azure_storage_connection_string
    ) as client:
        blob_client = client.get_blob_client(
        ...
        try:
            ...
            existing: list = json.loads(raw.decode("utf-8"))
        except ResourceNotFoundError:
            existing = []

        existing.append(clip_url)
        content = json.dumps(existing, ensure_ascii=False).encode("utf-8")
        await blob_client.upload_blob(content, overwrite=True, content_type="application/json")
        logger.debug(
            "append_to_clip_list: job=%s session=%s now %d clips", job_id, session_id, len(existing)
        )
        return blob_client.url
