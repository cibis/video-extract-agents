...
async def write_query_asset(payload: dict[str, Any]) -> dict[str, Any]:
    """Download a blob asset, apply JSONPath, and write the filtered result to a new blob.
    ...
    Output:
      result_asset: str — blob URL containing the matches array — pass as frames_asset to detect_* tools
      matches: list — matched values (capped at max_results)
      total_matches: int — total number of matches before capping
      truncated: bool — true if total_matches > max_results
    """
    ...
    try:
        ...
        logger.warning("write_query_asset: could not read blob %s: %s", blob_url, exc)
        return {"error": f"Could not read asset: {exc}"}
    ...
    try:
        ...
        logger.warning("write_query_asset: JSONPath error for '%s': %s", jsonpath_expr, exc)
        return {"error": f"JSONPath error: {exc}"}
