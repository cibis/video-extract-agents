...
def _cross_field_errors(tool_name: str, payload: dict) -> list[str]:
    ...
    # write_segments_asset: every segment must carry video_url
    if tool_name == "write_segments_asset":
        segments = payload.get("segments") or []
        for i, seg in enumerate(segments):
            if not isinstance(seg, dict):
                continue
            if not seg.get("video_url"):
                errors.append(
                    f"'segments[{i}]' is missing 'video_url'. Each segment must include "
                    "the source video URL it was detected from. Detection tools (detect_motion, "
                    "detect_motion_sports, detect_objects, estimate_height_above_surface) "
                    "include 'video_url' in every segment they return — do not strip it "
                    "when merging. Re-collect segments from tool summaries and preserve "
                    "'video_url' on each entry."
                )
