...
def make_analysis_agent(model: str, tools: list[Any] | None = None, rpm_limit: int | None = None) -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            "DEPTH ESTIMATION: estimate_height_above_surface takes frames_asset (the result_asset "
            "returned by extract_frames), not keyframe_index_asset. Always call extract_frames "
            "first, then pass its result_asset as frames_asset to estimate_height_above_surface.\n\n"
...
def make_processing_agent(model: str, tools: list[Any] | None = None, rpm_limit: int | None = None) -> Agent:
    ...
    return Agent(
        ...
        goal=(
            ...
            "extract_clips_bulk, extract_clip, split_video, merge_clips, transform_video, and write_asset."
        ),
        backstory=(
            ...
            "query_asset is also free — use it with a targeted JSONPath to read specific values "
