...
def make_analysis_task(
    ...
    return Task(
        description=(
            ...
            "JSONPath (e.g. '$.segments[*]' or '$.frames[*].timestamp_seconds') "
            ...
            "Each detection and motion tool returns a result_asset blob URL and a summary with "
            "aggregate counts only (segments_count, etc.) — the summary does NOT contain the "
            "segments array. After all detection tools have run, retrieve segments from each "
            "result_asset by calling query_asset(blob_url=<result_asset>, jsonpath='$.segments[*]', "
            "max_results=500). Each segment already contains 'video_url' — do NOT strip or omit it. "
            "If a tool's summary.segments_count is 0, skip the query_asset call for that tool.\n\n"
            "Never expand or read 'result_asset' blobs in your reasoning except via query_asset "
            "with a targeted JSONPath.\n\n"
            "After collecting segments from all query_asset calls across all tools and all videos, "
            "merge them into a single deduplicated list ordered by start_seconds. "
